...
describe("parseSubscriptions", () => {
  it(`given no latest receipt info, expect empty`, async () => {
    ...
    expect(parseSubscriptions(response.latest_receipt_info, response.pending_renewal_info, response.receipt.in_app)).toEqual([])
  })
  it(`given subscription transactions, expect get parsed subscription responses`, async () => {
    ...
    const actual = parseSubscriptions(response.latest_receipt_info, response.pending_renewal_info, response.receipt.in_app)
